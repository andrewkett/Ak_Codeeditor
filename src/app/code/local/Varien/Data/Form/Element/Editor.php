    ...
    public function getElementHtml()
    {
        ...
        if($this->isEnabled())
        {
            ...
            $html = $this->_getButtonsHtml()
               ...
               // .'<pre id="'.$this->getHtmlId().'_editor" style="height: 200px;">'.$this->getEscapedValue().'</pre>'
...
//    **/
    ...
    eManager.attachEditorToTextarea("'.$this->getHtmlId().'");

    varienGlobalEvents.attachEventHandler("tinymceSaveContent", function(){
        eManager.getEditor("'.$this->getHtmlId().'").container.show()
        setTimeout(function(){
            eManager.getEditor("'.$this->getHtmlId().'").setValue($("'.$this->getHtmlId().'").value);
        }, 50);
    });

    varienGlobalEvents.attachEventHandler("tinymceBeforeSetContent", function(){
        eManager.getEditor("'.$this->getHtmlId().'").container.hide()
    });
...
//    {
//
//        $html = '<script src="/js/ace/src-noconflict/ace.js" type="text/javascript" charset="utf-8"></script>';
//
...
//        $html .= "</textarea>";
//        $html .= '<pre id="'.$this->getName().'_editor" style="height: 200px;">'.$this->getEscapedValue().'</pre>';
//
//        $html .= $this->getAfterElementHtml();
//
//        $html .= '<script>
//    var editor = ace.edit("'.$this->getName().'_editor");
//    var textarea = $("'.$this->getHtmlId().'").hide();
